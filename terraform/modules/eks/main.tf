...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    security_group_ids      = [var.sg_eks_cluster_id]
  }
...
resource "aws_eks_node_group" "main" {
  ...
  node_group_name = "${local.name_prefix}-eks-node-grp"
...
# --- IRSA: API pod role (reads Secrets Manager) ---
resource "aws_iam_role" "api_pod" {
  name = "${local.name_prefix}-api-pod-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      ...
      Condition = {
        StringEquals = {
          "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${var.k8s_namespace}:api"
...
resource "aws_iam_role_policy" "api_pod_secrets" {
  name = "${local.name_prefix}-api-pod-secrets-policy"
